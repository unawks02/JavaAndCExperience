...
// TA: Surabhi Gupta
// Lecturer: Gary Dahl
// Notes to Grader: I used an internal helper class, 
// I feel like I implemented this a weird way. but it works.
// This was the way that felt intuitive to me.
...
public class CS400Graph<T> implements GraphADT<T> {
  ...
  protected class Path implements Comparable<Path> {
    ...
    public Path(Path copyPath, Edge extendBy) {
      ...
      for (int i = 0; i < copyPath.dataSequence.size(); i++) {
        this.dataSequence.add(copyPath.dataSequence.get(i)); //copies over data from copyPath
      }
      this.dataSequence.add(extendBy.target.data);
      this.end = extendBy.target; //adds new edge
  ...
  protected Path dijkstrasShortestPath(T start, T end) throws NoSuchElementException{
    if((start == null) || (end == null)) {
      throw new NoSuchElementException("No null arguments allowed");
      //throws exceptions if start or end data not found in graph
    }
    PriorityQueue<Path> discoveredPaths = new PriorityQueue<Path>();
    /*creates a PriorityQueue that holds the discovered paths 
     * (including total cost distance, and previous vertex data) that have not yet been determined 
     * to be the shortest possible paths to their given end vertex
     */
    VertexCost startAt = null;
    Vertex startingVertex = null;
    VertexCost endAt = null;
    LinkedList<VertexCost> vertexCosts = new LinkedList<VertexCost>(); 
    // creates a linked list storing Vertexes in their graphs and the least cost to them
    for (Vertex v : vertices.values()) {
      if ((v.data).equals(start)) {
        startingVertex = v;
        startAt = new VertexCost(v);
        vertexCosts.add(startAt);
        if ((v.data).equals(endAt)) {
          endAt = startAt;
        }
      }
      else if ((v.data).equals(end)) {
        endAt = new VertexCost(v);
        vertexCosts.add(endAt);
      }
      else {
      vertexCosts.add(new VertexCost(v));
      }
    }
    if((startAt == null) || (startingVertex == null)) {
      throw new NoSuchElementException("Start data not found in graph");
      //throws exceptions if start data not found in graph
    }
    if(endAt == null) {
      throw new NoSuchElementException("End data not found in graph");
      //throws exceptions if end data not found in graph
    }
    
    Path startIsEnd = new Path(startingVertex);
    if (startingVertex.data.equals(end)) {
      return startIsEnd;
    }
    
    //start at start and go to its connections
    PriorityQueue<Path> toVisit = new PriorityQueue<Path>();
      for (Edge e : startAt.getVertex().edgesLeaving) {
        //looks through nodes start connects to
        Vertex destination = e.target;
        for (int i = 0; i < vertexCosts.size(); i++) {
          VertexCost current = vertexCosts.get(i);
          if (current.getVertex().equals(destination)) {
            //matches VertexCost list to node starts connects to
            Path pathToVertex = new Path(startIsEnd, e);
            if (pathToVertex.distance < current.getLeastCost()) {
              //keeps data of path if best available
              toVisit.add(pathToVertex); //note that this path needs to be visited
              current.setLeastCost(pathToVertex.distance);
              current.setPathTo(pathToVertex);
            }
          }
        }
      }
      
      startAt.visit(); //denotes starting node has been visited
      
      Path nextToVisit = toVisit.poll();
      
      //start visiting next nodes via edges found above
      while (nextToVisit != null) {
        Vertex next = nextToVisit.end;
        for (int i = 0; i < vertexCosts.size(); i++) {
          if (vertexCosts.get(i).getVertex().equals(next)) {
            VertexCost current = vertexCosts.get(i);
            if (current.getVertex().data.equals(end)) {
              discoveredPaths.add(current.getPathTo());
              // adds Path to list of current options to node
            }
            if (!(current.getVisited())) { // check to see if current value has been visited
              for (Edge e : next.edgesLeaving) {
                // looks through nodes it connects to
                Vertex destination = e.target;
                for (int j = 0; j < vertexCosts.size(); j++) {
                  VertexCost nextNode = vertexCosts.get(j);
                  if (nextNode.getVertex().equals(destination)) {
                    // matches VertexCost list to node start connects to
                    Path pathToVertex = new Path(nextToVisit, e);
                    //extends current path
                    if (pathToVertex.distance < nextNode.getLeastCost()) {
                      // keeps data of path if best available
                      toVisit.add(pathToVertex);
                      nextNode.setLeastCost(pathToVertex.distance);
                      nextNode.setPathTo(pathToVertex);
                    }
                  }
                }
              }
        ...
        nextToVisit = toVisit.poll(); //continues cycling
      }
      
    if (discoveredPaths.peek() == null) { //indicates no path found
      throw new NoSuchElementException("No path found from " + start + " to " + end);
    }
    
    return discoveredPaths.peek(); //returns first in priority queue (shortest)
  }
  
  /**
   * This class was made to help implement the Djikstra's shortest path algorithm.
   * It is a private, internal helper class.
   * It helps to store vertexes and the shortest cost to get to that vertexes.
   * 
   * @author Sage Fritz
   *
   */
  private class VertexCost{
    private Vertex vertex; //the vertex this stores the shortest path to
    private double leastCost; //the value of the shortest path to this vertex
    boolean visited; //tells whether this node is visited or not
    Path pathTo; //gives the current best path to this node
    
    /**
     * This constructor makes a Vertex with the default values:
     * least Cost is set to infinity, visited is set to false.
     * 
     * @param value the Vertex to be stored in this VertexCost instance
     */
    private VertexCost(Vertex value) {
      this.vertex = value;
      this.leastCost = Double.POSITIVE_INFINITY; //indicates no path here has been found
      this.visited = false;
    }
    
    /**
     * Returns the vertex stored in VertexCost
     * 
     * @return the vertex stored in this instance
     */
    ...
    /**
     * Returns the current least cost to travel to this node.
     * 
     * @return the least cost of getting to this node
     */
    ...
    /**
     * Returns the current best path to this node.
     * 
     * @return a Path object with the least cost to this node
     */
    ...
    /**
     * Change this instance's stored least cost to the parameter input
     * 
     * @param newValue the new Least Cost to store
     */
    private void setLeastCost(double newValue) {
      this.leastCost = newValue;
    }
    
    /**
     * Notes that this node has been visited, changing the visited value to true.
     * Visited boolean was set to true.
     */
    private void visit() {
      this.visited = true;
    }
    ...
    /**
     * Changes the current Shortest Path stored in this instance to the input parameter.
     * 
     * @param shortestPath the new least cost path to the vertex stored in this instance
     */
    private void setPathTo(Path shortestPath) {
      this.pathTo = shortestPath;
    }
  }
